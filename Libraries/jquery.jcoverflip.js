...
$.ui.jcoverflip.defaults = {
	...
	beforeCss: function( el, container, offset ){
		...
		if (hnew < 75) {
		//If when scale to width, height fits
		//scale to width
														//midpt - (big pic +10) - (n*small wid)
		return [
			$.jcoverflip.animationElement( el, { left: ( container.width( )/2 - 108 - 72 - 10 - 118*offset )+'px', bottom: '20px' }, { } ),
			$.jcoverflip.animationElement( el.find( 'img' ), { opacity: 0.5, width: '108px' }, {} )
		];
		
		} else {
		//If scaling to width too big
		//Scale to max height that fits
		var wnew = w*75/h;
		var adjust = (108-wnew)/2;
		
		return [
			$.jcoverflip.animationElement( el, { left: ( container.width( )/2 -108 + adjust - 72 - 10 - 118*offset )+'px', bottom: '20px' }, { } ),
			$.jcoverflip.animationElement( el.find( 'img' ), { opacity: 0.5, height: '75px' }, {} )
		];
	...
	afterCss: function( el, container, offset ){
		...
		if((container.width( )/2 + 72 + 10 + 118*offset) < container.width( )) {
			return [
				$.jcoverflip.animationElement( el, { left: ( container.width( )/2 + 72 + 10 + 118*offset)+'px', bottom: '20px' }, { } ),
				$.jcoverflip.animationElement( el.find( 'img' ), { opacity: 0.5, width: '108px' }, {} )
			];
		} else {
			return [
				$.jcoverflip.animationElement( el, { left: ( -10000)+'px', bottom: '20px' }, { } ),
				$.jcoverflip.animationElement( el.find( 'img' ), { opacity: 0.0, width: '108px' }, {} )
			];
		...
		//Otherwise scale to max fitting height
		var wnew = w*75/h;
		var adjust = (108-wnew)/2;
		
		if((container.width( )/2 + 72 + 10 + adjust + 118*offset) < container.width( )) {
			return [
				$.jcoverflip.animationElement( el, { left: ( container.width( )/2 + 72 + 10 + adjust + 118*offset )+'px', bottom: '20px' }, { } ),
				$.jcoverflip.animationElement( el.find( 'img' ), { opacity: 0.5, height: '75px' }, {} )
			];
		} else {
			return [
				$.jcoverflip.animationElement( el, { left: ( -10000 )+'px', bottom: '20px' }, { } ),
				$.jcoverflip.animationElement( el.find( 'img' ), { opacity: 0.0, height: '75px' }, {} )
			];
	...
	currentCss: function( el, container ){
		...
		return [
			$.jcoverflip.animationElement( el, { left: ( container.width( )/2 - 72 )+'px', bottom: 0 }, { } ),
			$.jcoverflip.animationElement( el.find( 'img' ), { opacity: 1, width: '144px' }, { } )
		];} else {
		//Scale to max height
		var wnew = w*100/h;
		wnew = wnew/2;
		...
		return [
			$.jcoverflip.animationElement( el, { left: ( container.width( )/2 - wnew )+'px', bottom: 0 }, { } ),
			$.jcoverflip.animationElement( el.find( 'img' ), { opacity: 1, height: '100px' }, { } )
		];
